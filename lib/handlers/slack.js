...
const { openTaskModal, postMessage, getSlackUserEmail } = require('../slack');
const { getTodoistProjects, getSections, createTodoistTask, getTodoistIdByEmail } = require('../todoist');
...
async function handleBlockSuggestion(payload, res, workspaceId) {
  ...
  if (actionId === 'section_select') {
    // metadataからプロジェクトIDを取得
    console.log('[section_select] private_metadata:', payload.view?.private_metadata);
    let projectIds = [];
    if (payload.view?.private_metadata) {
      try {
        const meta = JSON.parse(payload.view.private_metadata);
        projectIds = meta.projects ? meta.projects.split(',') : [];
        console.log('[section_select] projectIds:', projectIds);
      } catch (e) {
        console.log('[section_select] parse error:', e.message);
      }
    }

    // プロジェクトが未選択の場合は「なし」のみ返す（高速レスポンス）
    if (projectIds.length === 0) {
      return res.status(200).json({
        options: [{ text: { type: 'plain_text', text: '（プロジェクトを先に選択）' }, value: 'none' }]
      });
    }

    // セクションを取得
    let options = [{ text: { type: 'plain_text', text: '（なし）' }, value: 'none' }];
    try {
      const [projects, ...sectionsResults] = await Promise.all([
        getTodoistProjects(),
        ...projectIds.map(id => getSections(id))
      ]);
      ...
      for (let i = 0; i < projectIds.length; i++) {
        ...
        const sections = sectionsResults[i] || [];
        const filtered = sections
          .sort((a, b) => (a.section_order || a.order || 0) - (b.section_order || b.order || 0))
      ...
      console.error('[section_select] Error:', err.message);
    }
...
async function handleBlockActions(payload, res, workspaceId, botToken) {
  ...
  // プロジェクト選択 → response_actionでmetadataを更新（views.update APIより確実）
  if (action?.action_id === 'project_select') {
    ...
    // response_action: 'update' でビューを更新（API呼び出し不要）
    return res.status(200).json({
...
async function handleViewSubmission(payload, res, workspaceId, botToken) {
  ...
  try {
    ...
    // メンションユーザーのTodoist IDを取得（メールで自動マッチング）
    const assignees = [];
    for (const slackUserId of mentionUserIds) {
      const email = await getSlackUserEmail(slackUserId, botToken);
      if (email) {
        const todoistId = await getTodoistIdByEmail(email);
        assignees.push({ slackUserId, todoistId, email });
      } else {
        assignees.push({ slackUserId, todoistId: null, email: null });
      }
    }

    // 担当者がいない場合は担当者なしで1回だけ作成
    const assigneeList = assignees.length > 0 ? assignees : [{ slackUserId: null, todoistId: null }];
        ...
        if (sectionsForProject.length > 0) {
          for (const secOption of sectionsForProject) {
            ...
            const taskOptions = {
              ...
              ...(assignee.todoistId && { responsible_uid: assignee.todoistId })
            };

            const task = await createTodoistTask(content, taskOptions);
            createdTasks.push({ task, projectName, sectionName, assigneeSlackId: assignee.slackUserId });
          }
        } else {
          const taskOptions = {
            ...(dueString && { due_string: dueString, due_lang: 'ja' }),
    ...
    if (botToken && notifyChannel) {
      ...
      msg += `作成数: ${createdTasks.length}件\n\n`;

      for (const { task, projectName, sectionName, assigneeSlackId } of createdTasks) {
        ...
        const assigneeText = assigneeSlackId ? ` (<@${assigneeSlackId}>)` : '';
        msg += `🗂️ ${hierarchy}${assigneeText} - <${taskUrl}|開く>\n`;
      }
