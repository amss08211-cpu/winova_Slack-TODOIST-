...
async function handleBlockActions(payload, res, workspaceId, botToken) {
  ...
  if (action?.action_id === 'project_select') {
    ...
    if (botToken) {
      ...
      // 非同期で実行（awaitしない）- レスポンスを速くするため
      updateModal(botToken, payload.view.id, payload.view, payload.view.blocks, metadata)
        .catch(err => console.error('[project_select] updateModal failed:', err.message));
    }
  }
