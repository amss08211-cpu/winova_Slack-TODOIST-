...
async function handleBlockSuggestion(payload, res, workspaceId) {
  ...
  if (actionId === 'section_select') {
    ...
    // まずstate.valuesから直接プロジェクトを取得（views.updateに依存しない）
    const selectedProjects = payload.view?.state?.values?.task_project?.project_select?.selected_options || [];
    if (selectedProjects.length > 0) {
      projectIds = selectedProjects.map(opt => opt.value);
    } else if (payload.view?.private_metadata) {
 ...
 * ユーティリティ: モーダル更新
 */
async function updateModal(botToken, viewId, currentView, blocks, metadata) {
  ...
  if (!result.ok) {
    console.error('[updateModal] Error:', result.error, result);
  }
}
