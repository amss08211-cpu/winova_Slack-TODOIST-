...
function buildTaskLine(task, projectName, slackUserId, options = {}) {
  ...
  let daysText = '';
  if (options.overdueDays > 0) {
    daysText = ` (${options.overdueDays}日超過)`;
  } else if (options.daysUntilDue > 0) {
    daysText = ` (あと${options.daysUntilDue}日)`;
  }

  return `${mention}📝 *${task.content}* - ${projectName}${daysText} <${taskUrl}|開く>`;
}
